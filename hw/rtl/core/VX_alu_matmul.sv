...
// `endif
    ...
    // wire [`INST_ALU_BITS-1:0] alu_op = `INST_ALU_BITS'(execute_if.data.op_type); // 4 bits
    // wire [`INST_BR_BITS-1:0]   br_op = `INST_BR_BITS'(execute_if.data.op_type); // 4 bits
    ...
    //for (genvar i = 0; i < NUM_LANES; ++i) begin : g_mat_mul_result
    //    assign mat_mult_results[i] = 'x;
    //end
    ...
    // wire [`XLEN-1:0] br_result = mat_mul_result_r[tid_r];
